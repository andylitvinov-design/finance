...
export default async function handler(request, response) {
  ...
  try {
    ...
    return response.status(400).json({
      ...
      skipped: 0,
      warnings: [],
      errors: [String(error?.message || error || "Revolut statement import failed.")],
      error: String(error?.message || error || "Revolut statement import failed."),
    });
  }
}
...
export function parseRevolutStatementRows(rowsOrText, options = {}) {
  ...
  const entries = rows.slice(headerIndex + 1)
    .map((row, index) => normalizeRevolutTransaction(row, index, { ...options, headerMap, warnings, skipped }))
...
export function normalizeRevolutTransaction(row, index = 0, options = {}) {
  ...
  if (isSkippedStatus(status)) {
    ...
    options.skipped?.push({ index, status, reason });
  ...
  const description = compactDescription([read("description"), read("type"), read("account")]);
