...
test("balance snapshots reads Остатки rows and warns about Факт now rows missing in Остатки", async () => {
  ...
  assert.equal(snapshot.balance_snapshots.diagnostics.fact_balance_rows_detected, 2);
  assert.equal(snapshot.balance_snapshots.diagnostics.fact_balance_rows_saved_to_ostatki, 1);
  assert.equal(snapshot.balance_snapshots.diagnostics.balance_snapshot_rows_loaded, 1);
  assert.equal(snapshot.balance_snapshots.diagnostics.skipped_non_balance_fact_rows, 0);
  assert.equal(snapshot.balance_snapshots.diagnostics.saved_fact_balance_rows[0].normalized_key, "2026-05-17|трансервайз дол|USD");
  assert.equal(snapshot.balance_snapshots.diagnostics.skipped_fact_balance_rows[0].normalized_key, "2026-05-17|пейпал дол|USD");
  ...
  assert.ok(snapshot.balance_snapshots.fact_balance_rows.some((row) =>
    ...
    && row.currency === "USD"
    && row.normalized_key === "2026-05-17|пейпал дол|USD"
