...
test("ledger amount_net migration preserves safe amount sign through operation semantics", () => {
  const values = [
    ["date", "operation", "from_channel", "to_channel", "amount", "currency", "amount_usd", "amount_gross", "amount_fee", "amount_net", "source", "raw_source_id"],
    ["2026-05-01", "income", "", "cash usd", "75", "USD", "75", "75", "", "", "manual", "manual-income"],
    ["2026-05-02", "expense", "cash usd", "", "-42", "USD", "-42", "42", "", "", "manual", "manual-expense"],
  ];

  const result = buildMigratedLedger(values);
  const report = toReport(result);

  assert.equal(report.missingAmountNetRows, 2);
  assert.equal(report.safeBackfilledRows, 2);
  assert.equal(result.values[1][9], "75");
  assert.equal(result.values[2][9], "42");
  assert.equal(report.balanceDelta, 0);
});
