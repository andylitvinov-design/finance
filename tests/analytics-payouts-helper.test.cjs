...
test("buildMovementPaymentSummaryRows falls back blank clients to transferwise and paypal", () => {
  const rows = buildMovementPaymentSummaryRows(
    [
      ...
      ["18149", "Инна Устименко", "", "100", "103", "72.1", "103", "0"],
      ["18151", "Инна Устименко", "сайт, дол", "5", "5.15", "3.605", "115.5", "-110.35"]
    ],
  ...
  assert.deepEqual(rows[1], ["пейпал дол", "170,0000", "175,1000", "122,5700", "175,1000", "0,0000"]);
  assert.deepEqual(rows[2], ["монобанк грн", "0,0000", "0,0000", "0,0000", "0,0000", "0,0000"]);
  assert.deepEqual(rows[3], ["Итого", "330,0000", "339,9000", "237,9300", "339,9000", "0,0000"]);
});
